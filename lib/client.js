...
const UI_ZH = {
		...
		function apply(ctx) {
			...
			// The card registered into `settings.plugin.item`, fed by
			// `settingsScope`. BOTH ARE GONE IN 0.1.7: the client service is now
			// `remote.settings`, and the per-namespace item slot no longer exists —
			// the Plugins page keeps only `settings.plugins.tab`. Registering into a
			// slot nothing offers is silently inert, and worse, declaring the removed
			// `settingsScope` in `inject` left this whole bundle PENDING forever,
			// which the host reports as
			//   "1 entry did not activate — waiting for service: settingsScope".
			//
			// This plugin therefore contributes exactly one surface: the management
			// page, which is also the one that works over a non-loopback address
			// because it reads this plugin's own host routes rather than settings.
