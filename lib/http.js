/**
 * dsh-skill-mcp-panel — the host's own HTTP data channel.
 *
 ...
 *
 * @module dsh-skill-mcp-panel/http
 */
...
export function createHttpRoutes(deps) {
  ...
  async function handleState(req, res) {
    try {
      ...
      sink(`skill-mcp-panel: building state failed: ${String(error)}`)
      sendJson(res, 500, {
  ...
  async function handleApply(req, res) {
    ...
    try {
      ...
      sink(`skill-mcp-panel: applying a batch failed: ${String(error)}`)
      sendJson(res, 500, {
  ...
  function mount(webServer) {
    ...
    try {
      ...
      sink(`skill-mcp-panel: could not register the HTTP routes: ${String(error)}`)
    ...
    }, 'skill-mcp-panel http routes')
    sink(`skill-mcp-panel: mounted ${ROUTE_STATE} and ${ROUTE_APPLY} (prefix ${HTTP_PREFIX}, deliberately outside /api)`)
  }
  ...
  ctx.inject(['webServer'], (webCtx) => {
    ...
    if (webServer === undefined || typeof webServer.register !== 'function') {
      sink('skill-mcp-panel: no webServer service; the management page has no data channel on this host')
      return
    }
    mount(webServer)
  })
