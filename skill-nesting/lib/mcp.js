/**
 * dsh-skill-nesting — MCP server inventory, toggling, and lifecycle.
 *
...
export function createMcpManager(deps = {}) {
	...
	function toolsFor(serverName) {
		...
		try {
			...
			log(`skill-nesting: could not read ctx.tools.schemas(): ${String(error)}`)
		}
	...
	async function loadClientModule() {
		...
		try {
			...
			log(`skill-nesting: ${MCP_CLIENT_MODULE} is not available: ${String(error)}`)
			clientModule = false
		}
		return clientModule
	}
	...
	async function readLoaderRows() {
		...
		if (manager !== undefined && typeof manager.listPlugins === 'function') {
			...
			try {
				...
				log(`skill-nesting: pluginManager.listPlugins() failed: ${String(error)}`)
			}
		}
		...
		try {
			...
			log(`skill-nesting: could not enumerate loader entries: ${String(error)}`)
			return out
		}
	...
	async function describe() {
		...
		try {
			...
			log(`skill-nesting: could not read MCP declarations: ${String(error)}`)
		}
